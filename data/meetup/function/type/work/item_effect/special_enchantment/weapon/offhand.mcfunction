...
#轉移該物品ID
data modify entity @e[type=item_display,sort=nearest,limit=1,distance=..1,tag=tmp_weapon_item] item.id set from entity @s Inventory[{Slot:-106b}].id
#轉移數量
data modify entity @e[type=item_display,sort=nearest,limit=1,distance=..1,tag=tmp_weapon_item] item.count set from entity @s Inventory[{Slot:-106b}].count
#轉移所有components
data modify entity @e[type=item_display,sort=nearest,limit=1,distance=..1,tag=tmp_weapon_item] item.components set from entity @s Inventory[{Slot:-106b}].components
#設定手上的物品的 stored ench
data modify entity @e[type=item_display,sort=nearest,limit=1,distance=..1,tag=tmp_weapon_item] item.components."minecraft:stored_enchantments" merge from entity @s Inventory[{Slot:-106b}].components."minecraft:enchantments"
#刪除可食用
...
#將暫存器儲存的物品轉移到玩家的手上
item replace entity @s weapon.offhand from entity @e[type=item_display,sort=nearest,limit=1,distance=..1,tag=tmp_weapon_item] contents
#刪除暫存器
